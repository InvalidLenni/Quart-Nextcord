...
class User(DiscordModelsBase):
    ...
    def __init__(self, payload):
        ...
        self._guilds = None         # Mapping of guild ID to quart_nextcord.models.Guild(...).
        self.connections = None     # List of quart_nextcord.models.UserConnection(...).

    @property
    def guilds(self):
        """A cached mapping of user's guild ID to :py:class:`quart_nextcord.Guild`. The guilds are cached when the first
    ...
    async def fetch_from_api(cls, guilds=False, connections=False):
        ...
        current_app.nextcord.users_cache.update({self.id: self})
    ...
    def get_from_cache(cls):
        ...
        """
        return current_app.nextcord.users_cache.get(current_app.nextcord.user_id)
    ...
    async def add_to_guild(self, guild_id) -> dict:
        ...
        Raises
        ------
        quart_nextcord.Unauthorized
            Raises :py:class:`quart_nextcord.Unauthorized` if current user is not authorized.

        """
        try:
            data = {"access_token": (await current_app.nextcord.get_authorization_token())["access_token"]}
        except KeyError:
    ...
    async def fetch_guilds(self) -> list:
        ...
        -------
        list
            List of :py:class:`quart_nextcord.Guilds` instances.
    ...
    async def fetch_connections(self) -> list:
        ...
        -------
        list
            A list of :py:class:`quart_nextcord.UserConnection` instances.
