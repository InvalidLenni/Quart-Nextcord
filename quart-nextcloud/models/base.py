...
class DiscordModelsBase(metaclass=DiscordModelsMeta):
    ...
    async def _request(*args, **kwargs):
        """A shorthand to :py:func:quart_nextcord.request`. It uses Quart current_app local proxy to get the
        Quart-Nextcord client.

        """
        return await current_app.nextcord.request(*args, **kwargs)

    @staticmethod
    async def _bot_request(*args, **kwargs):
        """A shorthand to :py:func:quart_nextcord.bot_request`."""
        return await current_app.nextcord.bot_request(*args, **kwargs)
