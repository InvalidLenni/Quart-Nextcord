...
class DiscordOAuth2Session(_http.DiscordOAuth2HttpClient):
    ...
    This class inherits :py:class:`quart_nextcord._http.DiscordOAuth2HttpClient` class.
    ...
    async def create_session(
        ...
        data : dict, optional
            ...
            `:py:meth:quart_nextcord.DiscordOAuth2Session.callback` to retrieve this data in your callback view.
        ...
        permissions: typing.Union[nextcord.Permissions, int], optional
        ...
        async with await self._make_session(scope=scope, state=state) as nextcord_:
            authorization_url, state = nextcord_.authorization_url(configs.DISCORD_AUTHORIZATION_BASE_URL)
        ...
        if "bot" in scope:
            if not isinstance(permissions, (nextcord.Permissions, int)):
                raise ValueError(f"Passed permissions must be an int or nextcord.Permissions, not {type(permissions)}.")
    ...
    async def get_authorization_token() -> dict:
        ...
        previously by `:py:meth:`quart_nextcord.DiscordOAuth2Session.save_authorization_token` from user's cookies.
    ...
    async def authorized(self):
        ...
        async with await self._make_session() as nextcord_:
            return nextcord_.authorized
    ...
    async def fetch_connections() -> list:
        ...
        -------
        list
            List of :py:class:`quart_nextcord.models.UserConnection` objects.
    ...
    async def fetch_guilds(use_cache=True) -> list:
        ...
        -------
        list
            List of :py:class:`quart_nextcord.models.Guild` objects.
