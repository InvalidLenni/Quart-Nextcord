...
class DiscordOAuth2HttpClient(abc.ABC):
    ...
    This class is meant to be overridden by :py:class:`quart_nextcord.DiscordOAuth2Session` and should not be
    used directly.
    ...
    async def _fetch_token(self, state):
        async with await self._make_session(state=state) as nextcord:
            return await nextcord.fetch_token(
    ...
    async def request(self, route: str, method="GET", data=None, oauth=True, **kwargs) -> typing.Union[dict, str]:
        ...
        quart_nextcord.RateLimited
            Raises an instance of :py:class:`quart_nextcord.RateLimited` if application is being rate limited by Discord.

        """
        route = configs.DISCORD_API_BASE_URL + route
        async with await self._make_session() as nextcord:
            async with (
                    await nextcord.request(method, route, data, **kwargs)
